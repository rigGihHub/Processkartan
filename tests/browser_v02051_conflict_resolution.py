...
def run():
    ...
    html=storage+html
    a=Path('/tmp/maplini_doc_02051_a.txt');b=Path('/tmp/maplini_doc_02051_b.txt')
    ...
    with sync_playwright() as p:
        ...
        page.locator('#p48-new').click();page.locator('#p48-new-process-name').fill('Konfliktlösning');page.locator('#p48-new-process-name').press('Enter');page.wait_for_timeout(70)
        ...
        assert page.locator('#p48-doc-conflicts').is_visible()
        cards=page.locator('.p48-doc-conflict-card');assert cards.count()>=2
        assert page.locator('#p48-doc-create').is_disabled()
        head=page.locator('.p48-doc-conflict-head').inner_text();assert 'kvar' in head
        for i in range(cards.count()):
            card=cards.nth(i)
            if card.locator('.p48-doc-conflict-custom input').count():
                card.locator('.p48-doc-conflict-custom input').fill('Processägare')
                card.locator('.p48-doc-conflict-custom button').click()
            else:
                card.locator('.p48-doc-conflict-options button').first.click()
            page.wait_for_timeout(40)
        assert not page.locator('#p48-doc-create').is_disabled()
        assert '0 av' in page.locator('.p48-doc-conflict-head').inner_text()
        ...
        browser.close()
    print('browser v0.20.52 document conflict resolution ok')
