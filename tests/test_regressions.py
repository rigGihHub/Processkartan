...
def test_v0151_first_time_empty_canvas_contract():
    ...
    assert 'id="p48-empty-object"' not in APP
    assert 'id="p48-empty-activity"' not in APP
    assert 'Vad startar processen?' not in APP
    assert 'function refreshEmptyState()' in APP
    assert 'emptyState.hidden=true;' in APP
...
def test_v0171_auto_clean_process_chooses_direction_and_preserves_logic():
    ...
    assert "const ok=smartLayout('all',orientation,ids);" in APP
    assert "polishAutomaticConnectedLinks(ids,{forceAuto:true});" in APP
...
def test_v0185_walkthrough_history_and_followup():
    ...
    assert "Ange namn eller initialer så att genomgången kan följas upp." in APP
...
def test_v0192_visual_flow_polish_contract():
    ...
    assert 'Klart · flödet är upprätat, jämnt fördelat och pilarna har snyggats till' in APP
...
def test_v0207_direct_walkthrough_contract():
    ...
    assert "},360);" in APP
    assert "Ja eller Nej tar dig direkt vidare" in APP
