...
def test_v01037_background_library_controls():
    html=_template()
    assert 'APP_VERSION = "0.16.4"' in APP
    # v0.16.4 keeps a focused picker while preserving rendering support for legacy types.
...
def test_v01040_node_visual_style_controls_exist():
    ...
    # v0.16.4 keeps 3D/Glass rendering for legacy processes but no longer offers them by default.
...
def test_v01045_insert_step_on_link_is_wired():
    text = APP
    assert 'APP_VERSION = "0.16.4"' in text
...
def test_v0162_scale_process_is_top_level_and_stays_open():
    ...
    # v0.16.4 replaces +/- with a persistent slider that keeps the menu open.
    assert 'id="p48-process-scale"' in APP
