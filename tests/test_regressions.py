...
def test_v01026_cloud_save_uses_stable_snapshot():
    chunk=APP[APP.index("async function guardedProcessCloudWrite"):APP.index("async function loadCloudProcesses")]
...
def test_v01031_shared_view_persist_is_ephemeral():
    ...
    shared=chunk[chunk.index("if(sharedView)"):chunk.index("const changed=MapliniSyncCore.contentChanged")]
...
def test_v01037_background_library_controls():
    html=_template()
    assert 'APP_VERSION = "' in APP
    # v0.19.1 keeps a focused picker while preserving rendering support for legacy types.
...
def test_v01040_node_visual_style_controls_exist():
    ...
    # v0.19.1 keeps 3D/Glass rendering for legacy processes but no longer offers them by default.
...
def test_v01043_smart_connector_routing_controls_and_geometry():
    ...
    assert "function linkGeometry(link,A,B,{avoidConnectorCrossings=true,routingObstacles=null,routingSegments=null}={})" in html
...
def test_v01044_connector_labels_and_decision_defaults():
    ...
    assert 'function linkLabelElement(st,points,selected=false,occupiedLabels=[])' in APP
...
def test_v0112_quick_next_step_contract():
    ...
    assert 'MapliniEditingCore.smartNextStepPosition' in APP
    assert 'links.push(MapliniConnectorCore.create(sourceId,id,nextStepConnectorSide(pos.direction||direction),{label:autoLabel}))' in APP
    assert "requestAnimationFrame(()=>beginInlineEdit(el))" in APP
    assert "if(e.key==='Enter'&&!e.shiftKey)" in APP


def test_v0193_smart_next_build_contract():
    assert 'APP_VERSION = "' in APP
    assert 'function nextStepFlowDirection(sourceId)' in APP
    assert 'MapliniEditingCore.smartNextStepPosition' in APP
    assert "branchIndex:source.data.type==='decision'?outgoingCount:null" in APP
    assert 'nextStepConnectorSide(pos.direction||direction)' in APP
...
def test_v0120_smart_layout_integration():
    ...
    assert 'function smartLayout(scope,orientation,explicitIds=null)' in APP
...
def test_v0145_stability_cleanup_contract():
    ...
    assert "nodeQuickAlign.forEach(btn=>btn.addEventListener('click'" in APP
    assert "nodeQuickDistribute.forEach(btn=>btn.addEventListener('click'" in APP
...
def test_v0146_free_connector_and_visual_polish_contract():
    ...
    assert "MapliniConnectorCore.setFreeOffset(links,index,baseFreeDx+dx,baseFreeDy+dy)" in APP
    assert "setLinkStyle(i,{routing,anchorMode:'auto',viaX:null,viaY:null,freeDx:0,freeDy:0,autoManaged:true})" in APP
...
def test_v0151_first_time_empty_canvas_contract():
    ...
    # v0.20.47 deliberately restores a lightweight empty-canvas starter,
    # but not the old onboarding/tutorial card.
    assert 'id="p48-empty-first-text"' in APP
    assert 'id="p48-empty-activity"' in APP
    assert 'id="p48-empty-object"' in APP
    assert 'id="p48-empty-start"' in APP
    assert 'Vad händer först?' in APP
...
def test_v0152_new_process_dialog_replaces_browser_prompt():
    ...
    assert "newProcessName.value=''" in APP
    assert "newProcessName.placeholder='Ex. Hantera kundfaktura'" in APP
    assert "requestAnimationFrame(()=>newProcessName.focus())" in APP
...
def test_v0154_selected_connector_affordances_do_not_stack():
    assert 'APP_VERSION = "' in APP
    assert 'function linkQuickPlacement(points,st,labelPos=null)' in APP
...
def test_v0156_context_panel_titles_match_selection():
    ...
    assert "formatTitle.textContent=context==='link'?'Redigera pil'" in APP
    assert "context==='multi'?'Redigera markering'" in APP
    assert "context==='node'?'Redigera steg':'Verktyg'" in APP
...
def test_v01511_first_view_canvas_calm_and_runtime_banner_policy():
    assert 'APP_VERSION = "' in APP
    html = _template()
    ...
    # Page guides stay available for export, but do not cover the drawing canvas by default.
    pdf_start = html.index('<select id="p48-pdf-view"')
    pdf_end = html.index('</select>', pdf_start)
    pdf_html = html[pdf_start:pdf_end]
    assert '<option value="off" selected>Ingen PDF-yta</option>' in pdf_html
    assert '<option value="A4P">A4 stående</option>' in pdf_html
...
def test_v0162_scale_process_remains_available_after_progressive_disclosure():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    scale=soup.select_one("#p48-scale-menu")
    assert scale is not None
    assert "p48-scale-menu" in scale.get("class",[])
    assert scale.find_parent(id="p48-more-menu") is not None
...
def test_v0165_magnetic_alignment_and_properties_label():
    ...
    assert "const alignedH=Math.abs(ay-by)<=10,alignedV=Math.abs(ax-bx)<=10;" in APP
...
def test_v0167_smart_connector_polish_prefers_straight_axis_and_preserves_manual_routes():
    ...
    assert "if(!forceAuto&&(st.routing==='free'||st.anchorMode!=='auto'||(!st.autoManaged&&!legacyAuto)))continue;" in APP
    assert "const routing=(alignedH||alignedV)?'straight':'orthogonal';" in APP
    assert "setLinkStyle(i,{routing,anchorMode:'auto',viaX:null,viaY:null,freeDx:0,freeDy:0,autoManaged:true})" in APP
...
def test_v0171_auto_clean_process_chooses_direction_and_preserves_logic():
    ...
    assert 'MapliniLayoutCore.smartLayout(rects,internalLinks' in APP
    assert "polishAutomaticConnectedLinks(ids,{forceAuto:true});" in APP
...
def test_v0172_atomic_undo_for_destructive_and_layout_operations():
    ...
    assert "const ok=applyNodePositions(positions,label,false)" in APP
...
def test_v0183_quick_page_settings_are_visible_and_synced():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert soup.select_one("#p48-page-quick") is not None
    assert soup.select_one("#p48-page-quick-summary") is not None
    assert soup.select_one("#p48-page-format-quick") is not None
    assert soup.select_one("#p48-page-count-quick") is not None
    assert "function refreshPageQuickUi()" in APP
    assert "function applyPageSettings(nextView,nextCount)" in APP
    assert "A3 stående" in html and "A3 liggande" in html
    assert 'option value="8">8 sidor' in html


def test_v0184_walkthrough_and_sidebar_scroll_and_safe_auto_clean():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert soup.select_one("#p48-walkthrough-launch") is not None
    assert soup.select_one("#p48-walkthrough-panel") is not None
    assert soup.select_one("#p48-check-question-list") is not None
    assert soup.select_one("#p48-add-check-question") is not None
    assert "__MAPLINI_WALKTHROUGH_CORE__" in html
    assert "function openWalkthrough()" in APP
    assert "function commitWalkthroughStep(nextId)" in APP
    assert "function connectedNodeIds(" in APP
    assert "Koppla ihop minst två rutor först" in APP
    assert "Do not change the user's zoom/view automatically" in APP
    assert ".p48-side::after" in html
    assert "scroll-padding-bottom:140px" in html


def test_v0185_walkthrough_history_and_followup():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert soup.select_one("#p48-walkthrough-history-list") is not None
    assert soup.select_one("#p48-walkthrough-history-count") is not None
    assert soup.select_one("#p48-walkthrough-saved-note") is not None
    assert "const WALKTHROUGH_RUNS_KEY='maplini_walkthrough_runs_v1';" in APP
    assert "function saveCompletedWalkthroughRun()" in APP
    assert "function renderWalkthroughHistory()" in APP
    assert "function updateWalkthroughRunStatus(id,status)" in APP
    assert 'Namn / initialer <span class="p48-walkthrough-optional">(valfritt)</span>' in APP
    assert "||'Anonym'" in APP
    assert "Genomgångar sparas alltid lokalt." in html


def test_v0186_smart_yes_no_routing():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert "Vägvalsfråga" in html
    assert "function renderWalkthroughNavigation(item)" in APP
    assert "MapliniWalkthroughCore.automaticRoute" in APP
    assert "Fortsätt på ${answerLabel}" in APP
    assert "Maplini väljer rätt väg automatiskt" in APP
    assert "ingen unik utgående pil märkt Ja" in APP
    assert "ingen unik utgående pil märkt Nej" in APP


def test_v0187_control_vs_route_question_semantics():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert "Kontrollfråga" in html
    assert "Vägvalsfråga" in html
    assert "Nej räknas som avvikelse." in APP
    assert "Ja och Nej är normala vägar" in APP
    assert "result.deviations.length" in APP
    assert "MapliniWalkthroughCore.currentDeviationCount" in APP
    assert "kind:q.kind||'control'" in APP


def test_v0188_deviation_followup_fields():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert "Vad har avvikit? *" in APP
    assert "Ansvarig för åtgärd *" in APP
    assert "Förfallodatum *" in APP
    assert "function walkthroughDeviationDetailsAreComplete(item)" in APP
    assert "Fyll i förklaring, ansvarig och förfallodatum för varje avvikelse." in APP
    assert "currentDeviationDetails" in APP
    assert "explanation:d.explanation||''" in APP
    assert "owner:d.owner||''" in APP
    assert "dueDate:d.dueDate||''" in APP


def test_v0189_cloud_walkthrough_sync_contract():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert soup.select_one("#p48-walkthrough-cloud-state") is not None
    assert "function loadCloudWalkthroughRuns()" in APP
    assert "function saveWalkthroughRunToCloud(record)" in APP
    assert "function updateWalkthroughRunStatusInCloud(record)" in APP
    assert "/rest/v1/walkthrough_runs" in APP
    assert "Sparad lokalt · molnsynk väntar på Supabase-migreringen v0.18.9." in APP
    assert "record.cloud" in APP

def test_v0189_supabase_walkthrough_migration_exists():
    sql=(ROOT/"supabase_schema_v0189.sql").read_text(encoding="utf-8")
    assert "create table if not exists public.walkthrough_runs" in sql
    assert "process_id text not null references public.processes(id) on delete cascade" in sql
    assert "alter table public.walkthrough_runs enable row level security" in sql
    assert 'create policy "walkthrough members read"' in sql
    assert 'create policy "walkthrough users create"' in sql
    assert 'create policy "walkthrough followup update"' in sql


def test_v0190_deviation_dashboard_contract():
    assert 'APP_VERSION = "' in APP
    html=_template()
    soup=BeautifulSoup(html,"html.parser")
    assert soup.select_one("#p48-deviation-panel") is not None
    assert soup.select_one("#p48-deviation-status") is not None
    assert soup.select_one("#p48-deviation-owner") is not None
    assert soup.select_one("#p48-deviation-overdue-only") is not None
    assert "function deviationRows()" in APP
    assert "function filteredDeviationRows()" in APP
    assert "function renderDeviationDashboard()" in APP
    assert "function loadCloudWalkthroughRunsForScope()" in APP
    assert "Försenad" in APP
    assert "Markera avvikelse hanterad" in APP


def test_v0191_individual_deviation_status_contract():
    assert 'APP_VERSION = "' in APP
    assert "function deviationStatusFor(run,deviation)" in APP
    assert "function updateWalkthroughDeviationStatus(runId,deviationIndex,status)" in APP
    assert "deviationIndex:i" in APP
    assert "Markera avvikelse hanterad" in APP
    assert "Öppna avvikelse igen" in APP
    assert "result:record.result||{}" in APP
    assert "deviations.forEach(x=>{if(x&&x.status!=='resolved'&&x.status!=='open')x.status=inheritedStatus})" in APP


def test_v0192_visual_flow_polish_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-auto-clean-top"' in APP
    assert "autoCleanTopBtn.addEventListener('click',()=>autoCleanProcess())" in APP
    assert "mainGap:132,crossGap:64" in APP
    assert 'Processen snyggades till · Ångra återställer tidigare layout' in APP
    assert 'Förhandsvisning · kontrollera layouten innan du använder den' in APP
    core=(ROOT/'maplini_layout_core.js').read_text(encoding='utf-8')
    assert 'Center each rank on the actual flow' in core
    assert 'function shiftLevel(r,targetCenter,strength)' in core
    assert 'branches are centered on their source' in core


def test_v0194_smart_decision_pair_is_wired():
    assert "function addDecisionBranches(sourceId)" in APP
    assert "pair.textContent='Ja + Nej'" in APP
    assert "addDecisionBranchNode(source,'Ja',0,direction)" in APP
    assert "addDecisionBranchNode(source,'Nej',1,direction)" in APP
    branch_chunk=APP[APP.index('function positionDecisionBranchNode'):APP.index("function addNextStepFromNode")]
    assert "{label}" in branch_chunk
    assert "positionDecisionBranchNode(source,el,label,pos,direction)" in branch_chunk
    assert "item.data.type==='decision')addDecisionBranches" in APP


def test_v0195_guided_walkthrough_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-walkthrough-trail"' in APP
    assert "function renderWalkthroughTrail(item)" in APP
    assert "Har du gjort detta?" in APP
    assert "__maplini_default_done__" in APP
    assert "q.quick" in APP
    assert ".p48-walkthrough-answer{min-height:54px" in APP


def test_v0196_connector_visual_polish_contract():
    assert 'APP_VERSION = "' in APP
    assert "function roundedOrthogonalPath(points,radius=10)" in APP
    assert "effective.routing==='orthogonal'?roundedOrthogonalPath(points,10)" in APP
    assert "function branchSemantic(value)" in APP
    assert "branch-yes rect" in APP
    assert "branch-no rect" in APP


def test_v0197_node_visual_hierarchy_contract():
    assert 'APP_VERSION = "' in APP
    assert "v0.19.7 – clearer visual hierarchy" in APP
    assert "border-left:5px solid #2b7b61" in APP
    assert "min-height:46px" in APP
    assert "background:var(--decision-bg,#fffaf0)" in APP
    assert "box-shadow:0 5px 14px rgba(103,78,25,.10)" in APP


def test_v0198_shape_presets_and_visual_rhythm_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-node-shape"' in APP
    assert "shapePreset" in APP
    assert "p48-shape-rectangle" in APP
    assert "p48-shape-rounded" in APP
    assert "p48-shape-pill" in APP
    assert "mainGap:132,crossGap:64" in APP


def test_v0199_quick_shape_toolbar_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-node-quick-shape"' in APP
    assert APP.count("data-quick-shape=") == 4
    assert "nodeQuickShapeChoices.forEach" in APP
    assert "updateStyle({shapePreset:shape})" in APP
    assert "p48-node-quick-shape-pop" in APP


def test_v0200_game_feel_contract():
    assert 'APP_VERSION = "' in APP
    assert "function celebrateCreatedNode(el)" in APP
    assert "p48-node-spawn" in APP
    assert "p48-step-enter" in APP
    assert "prefers-reduced-motion:reduce" in APP


def test_v0201_interaction_feel_without_rewards_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-flow-streak"' not in APP
    assert "registerFlowBuild" not in APP
    assert "flowBuildStreak" not in APP
    assert "p48-summary-win" not in APP
    assert "p48-dragging" in APP
    assert "p48-snapped" in APP
    assert "p48-connect-target" in APP
    assert "p48-temp-ready" in APP
    assert "p48-answer-hit" in APP


def test_v0202_navigation_feel_contract():
    assert 'APP_VERSION = "' in APP
    assert "function zoomAtClientPoint(next,clientX,clientY)" in APP
    assert "spacePanHeld" in APP
    assert "p48-space-pan-ready" in APP
    assert "p48-marquee-preview" in APP
    assert "e.ctrlKey||e.metaKey" in APP
    assert "e.button===1" in APP


def test_v0203_canvas_build_rhythm_contract():
    assert 'APP_VERSION = "' in APP
    assert "function createConnectedNodeAt(source,side,clientX,clientY)" in APP
    assert "p48-temp-create" in APP
    assert "dragDistance>70" in APP
    assert "e.key==='Tab'" in APP
    assert "e.key==='Enter'&&!mod" in APP
    assert "duplicateSelectedNodes()" in APP


def test_v0204_multi_selection_formation_contract():
    assert 'APP_VERSION = "' in APP
    assert 'id="p48-selection-hull"' in APP
    assert "function refreshSelectionHull()" in APP
    assert 'id="p48-node-quick-arrange"' in APP
    assert "data-node-quick-layout=" in APP
    assert "p48-group-dragging" in APP
    assert "['ArrowLeft','ArrowRight','ArrowUp','ArrowDown']" in APP


def test_v0205_smart_connector_flow_contract():
    assert 'APP_VERSION = "' in APP
    assert "st.autoManaged&&st.anchorMode==='auto'" in APP
    assert "const nearH=Math.abs(ay-by)<=10,nearV=Math.abs(ax-bx)<=10" in APP
    assert "autoManaged:false" in APP
    assert "p48-auto-live" in APP


def test_v0206_visual_flow_rhythm_contract():
    assert 'APP_VERSION = "' in APP
    assert "function flowSpacingFor(source,targetType,{branch=false}={})" in APP
    assert "gap=148;crossGap=76" in APP
    assert "crossGap=Math.max(crossGap,82)" in APP
    assert "function refreshSelectedFlowEmphasis()" in APP
    editing = (ROOT / "maplini_editing_core.js").read_text(encoding="utf-8")
    assert "const crossGap=Math.max(36,Number(o.crossGap)||54);" in editing


def test_v0207_direct_walkthrough_contract():
    assert 'APP_VERSION = "' in APP
    assert "function maybeAutoAdvanceWalkthrough(item,question,answer)" in APP
    assert "question.quick&&edges.length===1" in APP
    assert "},360);" in APP
    assert "Klart tar dig direkt vidare · Inte klart låter dig stanna upp" in APP
    assert "q.quick?' quick':''" in APP
    assert "quickAdvanceTimer" in APP


def test_v0207_focused_walkthrough_contract():
    assert 'APP_VERSION = "' in APP
    assert "function maybeAutoAdvanceWalkthrough(item,question,answer)" in APP
    assert "question.kind==='route'" in APP
    assert "else if(answer==='yes'&&edges.length===1)" in APP
    assert "function answerCurrentWalkthroughByKey(key)" in APP
    assert "keyHint.textContent=answer==='yes'?'J':'N'" in APP
    assert "text.textContent=single?q.text" in APP


def test_v0209_walkthrough_branch_badges_and_selected_route():
    assert 'APP_VERSION = "' in APP
    assert 'data-walk-route-label' in APP
    assert 'p48-walk-chosen' in APP
    assert 'p48-walk-link-chosen' in APP
    assert "next.el.dataset.walkRouteLabel=routeLabel" in APP

def test_v0209_walkthrough_visual_state_cleanup():
    assert "delete item.el.dataset.walkRouteLabel" in APP
    assert "'p48-walk-chosen'" in APP
    assert "'p48-walk-link-chosen'" in APP


def test_v02012_smart_branch_pair_layout_contract():
    assert 'APP_VERSION = "' in APP
    editing=(ROOT / "maplini_editing_core.js").read_text(encoding="utf-8")
    assert "function smartDecisionBranchPositions" in editing
    assert "smartDecisionBranchPositions" in APP
    assert "function addDecisionBranchPair(source,direction)" in APP
    assert "created.push(...addDecisionBranchPair(source,direction))" in APP
    assert "Ja- och Nej-grenar skapade symmetriskt" in APP
