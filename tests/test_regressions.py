...
def test_v01017_ui_core_is_loaded_and_injected():
    assert 'APP_VERSION = "0.15.10"' in APP
...
def test_v01037_background_library_controls():
    html=_template()
    assert 'APP_VERSION = "0.15.10"' in APP
    # v0.15.10 keeps a focused picker while preserving rendering support for legacy types.
...
def test_v01040_node_visual_style_controls_exist():
    ...
    # v0.15.10 keeps 3D/Glass rendering for legacy processes but no longer offers them by default.
...
def test_v01045_insert_step_on_link_is_wired():
    text = APP
    assert 'APP_VERSION = "0.15.10"' in text
